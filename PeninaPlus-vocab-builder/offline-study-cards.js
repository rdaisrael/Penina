...
input{width:100%;border:1px solid var(--line);border-radius:14px;padding:12px 13px;font-size:15px;outline:none}input:focus{border-color:var(--accent);box-shadow:0 0 0 4px var(--accent2)}button{border:1px solid var(--line);background:#fff;color:var(--ink);border-radius:14px;padding:11px 12px;font-weight:750;cursor:pointer}button:hover{transform:translateY(-1px);border-color:#b7c0d3}button.primary,button.active{background:var(--accent);border-color:var(--accent);color:#fff}.orders{display:grid;gap:9px;margin:14px 0}.actions,.controls,.ratings,.stats{display:grid;gap:9px}.actions{grid-template-columns:1fr 1fr}.controls,.ratings,.stats{grid-template-columns:repeat(3,1fr)}
...
<p class="section-title">Find a card</p><input id="search" placeholder="Search translations, term, or context…" autocomplete="off">
...
function order(card){const names=termFirst?['term','hebrew','english']:['hebrew','english','term'];return names.filter(name=>String(card[name]||'').trim())}
function renderList(){const query=byId('search').value.trim().toLowerCase();const visible=originalCards.filter(card=>!query||searchable(card).includes(query));cards=visible.filter(card=>ratings[card.n]!=='known');if(index>=cards.length)index=0;byId('count').textContent=cards.length;const active=cards[index]&&cards[index].n;byId('list').innerHTML=visible.map(card=>'<div class="list-item '+(ratings[card.n]||'')+' '+(card.n===active?'active':'')+'" data-card="'+card.n+'"><div class="li-top"><span>'+esc(card.english)+'</span><span>#'+card.n+'</span></div><div class="li-he">'+esc(card.term)+'</div></div>').join('');document.querySelectorAll('[data-card]').forEach(item=>item.onclick=()=>{const found=cards.findIndex(card=>card.n===Number(item.dataset.card));if(found>=0){index=found;sideIndex=0;render()}})}
...
function render(){renderList();renderStats();if(!cards.length){byId('position').textContent='0 / 0';byId('bar').style.width='0%';byId('mainText').textContent='No cards remaining';byId('sideLabel').textContent='';byId('sideNumber').textContent='';return}const card=cards[index],sides=order(card);if(sideIndex>=sides.length)sideIndex=0;const side=sides[sideIndex];byId('position').textContent=(index+1)+' / '+cards.length;byId('bar').style.width=((index+1)/cards.length*100)+'%';byId('cardNumber').textContent='Card #'+card.n;byId('sideNumber').textContent=(sideIndex+1)+' / '+sides.length;byId('sideLabel').textContent=side==='term'?'Term':side==='hebrew'?'Hebrew Translation of Term':'English Translation of Term';byId('mainText').className='main '+(side==='english'?'':'hebrew');byId('mainText').textContent=card[side]||'';byId('contextQuote').textContent=card.contextQuote;byId('hebrewContext').textContent=card.hebrewTranslation;byId('englishContext').textContent=card.englishTranslation;byId('hebrewSource').textContent=card.sourceHebrew;byId('englishSource').textContent=card.sourceEnglish}
