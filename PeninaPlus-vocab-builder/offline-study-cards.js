(function () {
    ...
    function mountAsteroids({ board, rows, createEngine, setFeedback, setProgress }) {
        ...
        const classConfig=doc.getElementById('peninaClassConfig');
        const boardUrl=classConfig&&window.location.protocol!=='file:'?JSON.parse(classConfig.textContent).leaderboardUrl:'';
        if(boardUrl)fetch(boardUrl).then(response=>{if(!response.ok)throw new Error();return response.json();}).then(data=>{if(!stopped)showBoard(data.scores);}).catch(()=>{if(!stopped)leaderboard.textContent='Class leaderboard is temporarily unavailable.';});
    ...
    // Serialized into downloaded HTML so all four games also work offline.
    function mountGames(groups, mountAsteroids, createAsteroids) {
        ...
        let stopAsteroids = null, matchingCount = 0;
        ...
        const labels = { quiz: 'Multiple Choice', lines: 'Connect the Words', cards: 'Match the Cards', asteroids: 'Vocabulary Asteroids' };
        ...
        function submitMatching(){
            ...
            form.onsubmit=async event=>{event.preventDefault();if(submit.disabled||!form.reportValidity())return;submit.disabled=true;status.textContent='Saving…';
                try{const response=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({...result,initials:input.value.toUpperCase()})});const data=await response.json();if(!response.ok)throw new Error(data.error||'Could not save your time.');if(token!==runToken)return;form.remove();status.textContent='Time saved! The top three times for this set appear on your class page.';}
                catch(error){if(token!==runToken)return;submit.disabled=false;status.textContent=error.message;}
            };
        }
        function updateProgress() {
            progress.textContent = mode === 'quiz' ? `${mastered.size} of ${totalTerms} terms mastered · ${attempts} attempts` : `${Math.min(rows.length,offset + matched.size)} of ${rows.length} pairs matched · ${attempts} attempts`+(mode==='cards'?` · ${((finishedMs||performance.now()-startedAt)/1000).toFixed(1)} seconds`:'');
        }
        function quiz() {
            ...
            shuffle([row.definition, ...row.answers]).forEach(answer => {
                const node = button(answer, () => {
                    ...
                    choices.querySelectorAll('button').forEach(choice=>{choice.disabled=true;if(choice.textContent===row.definition)choice.classList.add('matched');});
                    message('Correct! Great work.');updateProgress();next.hidden=false;next.focus();
                }, 'game-tile');
                choices.append(node);
            });
            board.append(choices);
            next.textContent = offset + 1 === rows.length ? 'See results' : 'Next question';
        ...
        function match(first, second) {
            ...
            if (matched.size === matchingCount) {
                board.querySelectorAll('.game-tile').forEach(node => {node.disabled=true;}); message('All terms matched! The extra definitions were distractors.'); if(mode==='cards'){if(offset+matchingCount===rows.length)finishedMs=Math.max(1,Math.round(performance.now()-startedAt));later(()=>{offset+=matchingCount;render();},450);}else{next.hidden = false; next.focus();}
            }
        }
        ...
        function center(node) {
            ...
            return { x: node.classList.contains('game-term') ? box.right : box.left, y: box.y + box.height / 2 };
        }
        function drawLines() {
            ...
            if (mode === 'cards') {
                ...
                items.forEach(item => {
                    item.node.onpointerdown = event => {
                        if (item.node.disabled || event.button !== 0) return;
                    ...
                    item.node.onpointerup = event => {
                        ...
                        else message('Drag from a term to a definition in the other column.');
        ...
        function start(value) {
            ...
            mode = value; rows = shuffle(groups[language.value]); offset = score = attempts = 0;
            ...
            if (mode === 'asteroids') {
                ...
                byId('gameHelp').textContent = 'Earn 10 fuel cells for every correct answer. Clear three rounds: asteroids, pursuing Martians, then enemy ships. Hits cost 150 points; wrong answers cost 25. Zero points means game over.';
            ...
            byId('gameHelp').textContent = mode === 'quiz' ? 'You have two chances. Miss twice and the term returns later.' : mode === 'lines'
                ? 'Match every term; extra definitions will remain unused. Drag a line from a term to its definition. You can also click or use Tab and Enter to select each pair.'
                : 'Race the clock! Correct pairs pop away. Match every term. Extra definitions will remain unused!';
        ...
        function menu() {
            ...
            byId('gameMenu').hidden = false; byId('gamePlay').hidden = true;
        ...
        dialog.querySelectorAll('[data-game]').forEach(node => { node.onclick = () => start(node.dataset.game); });
        byId('gameAgain').onclick = () => start(mode);
...
<dialog id="printOptionsDialog" class="print-options-dialog"><form class="print-options-form"><h2>Print Paper Flashcards</h2><p class="print-options-intro">Choose what will appear on the printed cards. Both languages print on the definition side. Your flip order sets the front and back; context prints only on the answer side.</p><fieldset class="print-option-group"><legend>Cards per sheet</legend><label class="print-option-choice"><input type="radio" name="cardsPerSheet" value="6">6 per sheet</label><label class="print-option-choice"><input type="radio" name="cardsPerSheet" value="8" checked>8 per sheet</label></fieldset><fieldset class="print-option-group"><legend>Context</legend><label class="print-option-choice"><input type="checkbox" id="printOriginalQuote" checked>Original quote</label><label class="print-option-choice"><input type="checkbox" id="printHebrewContext" checked>Hebrew translation</label><label class="print-option-choice"><input type="checkbox" id="printEnglishContext" checked>English translation</label></fieldset><div class="print-dialog-actions"><button id="closePrintOptions" type="button">Cancel</button><button id="confirmPrintOptions" class="primary" type="button">Preview Flashcards</button></div></form></dialog>
...
function getPrintSettings(){const layout=document.querySelector('input[name="cardsPerSheet"]:checked');return {count:Number(layout&&layout.value)===6?6:8,translation:'both',originalQuote:byId('printOriginalQuote').checked,hebrewContext:byId('printHebrewContext').checked,englishContext:byId('printEnglishContext').checked}}
...
<\/script>${gamesMarkup(groups)}
    ...
    const api = { download, makeApp, toCards, practiceRows, createAsteroids };
