...
function birdUI(){el('bird').dataset.bird=birdType;root.querySelectorAll('[data-choice]').forEach(b=>{const selected=b.dataset.choice===birdType;b.setAttribute('aria-pressed',String(selected));b.querySelector('.selection').textContent=selected?'Selected':'Choose';});}
...
function submitScore(){
 if(submitted||submitting||state==='ready')return;stop();state='over';
 panel('Submit score',score+' points','Start over →',start);
 if(!scoreUrl){el('feedback').textContent='Class score submission is available from your published class page.';return;}
 const form=document.createElement('form');form.className='submission';
 const input=document.createElement('input');input.required=true;input.maxLength=3;input.pattern='[A-Za-z]{1,3}';input.placeholder='1–3 initials';input.setAttribute('aria-label','Your initials');
 const button=document.createElement('button');button.type='submit';button.className='primary';button.textContent='Submit score';
 const status=document.createElement('p');status.setAttribute('role','status');form.append(input,button);el('overlay').append(form,status);
 form.onsubmit=async e=>{e.preventDefault();if(submitting||submitted)return;const initials=input.value.trim().toUpperCase();if(!/^[A-Z]{1,3}$/.test(initials)){status.textContent='Enter 1–3 letters.';return;}submitting=true;button.disabled=true;el('submit').disabled=true;const token=runId;
 try{const response=await fetch(scoreUrl,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({initials,score,runId}),signal:abort.signal});const data=await response.json();if(!response.ok)throw new Error(data.error||'Could not save score.');if(destroyed||token!==runId)return;submitted=true;form.remove();status.textContent='Score submitted!';el('submitted').textContent='Class top 3: '+data.scores.map(entry=>entry.initials+' — '+entry.score+' points').join(' · ');}
 catch(error){if(!destroyed&&token===runId){status.textContent=error.message;button.disabled=false;el('submit').disabled=false;}}
 finally{submitting=false;}};
 input.focus();
}

el('start').onclick=start;el('flap').onclick=flap;el('submit').onclick=submitScore;el('world').addEventListener('pointerdown',e=>{if(!e.target.closest('.overlay')){el('flap').focus({preventScroll:true});flap();}});// Capture Space throughout this preview, including when another game button has focus.
