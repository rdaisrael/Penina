...
async function topScores(grade) {
    const prefix = `asteroids-scores/${grade}/`;
    const scores = [];
    let cursor;
    do {
...
module.exports = async function (req, res) {
    ...
    try {
        ...
        if (req.method === 'POST') {
            const { initials, score, rounds, runId } = req.body || {};
            // The three rounds contain at most 8,040 available points including the starting score.
            if (typeof initials !== 'string' || !/^[A-Z]{1,3}$/.test(initials) || !Number.isInteger(score) || score <= 0 || score > 8040 || score % 5 || rounds !== 3 || typeof runId !== 'string' || !/^[a-zA-Z0-9-]{10,80}$/.test(runId)) {
                return res.status(400).json({ error: 'Enter 1–3 initials and a valid completed-game score.' });
            }
            const pathname = `asteroids-scores/${grade}/${String(score).padStart(4, '0')}-${initials}-${runId}.json`;
            try {
        ...
        console.error('Asteroids leaderboard:', error);
