...
async function room(code, shareRead = false) {
    if (!shareRead) return loadRoom(code);
...
function deadline(game) {
    ...
    return game.state.phase === 'question' ? game.state.at + game.config.seconds * 1000 : game.state.phase === 'reveal' ? game.state.at + 2000 : null;
}
...
async function snapshot(game, who, includeReveal = true) {
    ...
    const result = { code: config.code, className: config.className, classUrl: config.classUrl, phase: state.phase, version: state.version, index: state.index, total: config.questions.length, playerCount: roster.length, answeredCount: current.length, expiresAt: config.expiresAt, paused: !!state.paused, seconds: config.seconds || 0, deadline: deadline(game), serverNow: Date.now() };
...
module.exports = async function liveGame(req, res) {
    ...
    try {
        ...
        if (body.action === 'answer') {
            ...
            if (game.state.paused || game.state.phase !== 'question' || body.index !== game.state.index || !Number.isInteger(body.choice) || body.choice < 0 || body.choice > 3) fail(409, 'This question is closed or has changed.');
        ...
        const next = { version: game.state.version + 1, index: game.state.index, at: Date.now() };
        if (body.action === 'pause' && !game.state.paused && ['question','reveal'].includes(game.state.phase)) {
