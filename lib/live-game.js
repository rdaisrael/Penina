...
async function snapshot(game, who) {
    ...
    const cutoffs = new Map(reveals.filter(s => s.phase === 'reveal').map(s => [s.index, s.at]));
    ...
    const result = { code: config.code, className: config.className, classUrl: config.classUrl, phase: state.phase, version: state.version, index: state.index, total: config.questions.length, playerCount: roster.length, answeredCount: current.length, expiresAt: config.expiresAt };
...
async function create(body) {
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
        ...
        try { await write(prefix(code) + 'config.enc', { code, grade, className: page.name, classUrl: page.url, questions, expiresAt }); }
...
module.exports = async function liveGame(req, res) {
    ...
    try {
        ...
        if (body.action === 'answer') {
            ...
            try { await write(path, { id: who.id, index: body.index, choice: body.choice, at: Date.now() }); }
