...
function harness(){
 ...
 const board=new Element(),config=new Element();board.dataset.initials='AB';config.textContent=JSON.stringify({leaderboardUrl:'/api/game-scores?game=asteroids&grade=sixth'});
...
test('Space works with other buttons focused and leaving removes all listeners',()=>{const h=harness();h.control.start();h.listeners.get('keydown')({code:'Space',target:{tagName:'BUTTON'},preventDefault(){},stopPropagation(){}});assert.equal(h.control.get().readTime,0);assert.equal(h.control.get().vy,-140);assert(Math.abs(h.control.get().vy**2/540-36.2963)<.001);assert.equal(h.control.laneAt(111),-1);assert.equal(h.control.laneAt(0),0);assert.equal(h.control.laneAt(378),2);h.ctx.stop();assert(h.cancelled());assert.equal(h.listeners.size,0);});
test('Finished runs automatically save initials and score without a submit button',async()=>{const h=harness();h.control.start();for(let i=0;i<3;i++){h.control.resolve(-1);if(i<2)h.control.question();}await new Promise(resolve=>setImmediate(resolve));assert.equal(h.payload().body.score,-150);assert.equal(h.payload().body.initials,'AB');assert(h.payload().url.includes('game=flappy'));assert.equal(h.get('#fv-overlay').children.some(n=>n.className==='submission'),false);});
...
test('Restarting or leaving cancels a pending round countdown',()=>{
 for(const action of ['restart','leave']){
  const h=harness(),c=h.control;c.start();for(let i=0;i<5;i++)c.resolve(c.get().answers.indexOf('horse'));
  assert.equal(h.timers.size,1);
  if(action==='restart')c.start();else h.ctx.stop();
