...
test('Shared leaderboard route dispatches each game and rejects unknown games',async()=>{
 ...
 for(const game of ['asteroids','matching','unknown',undefined]){
  ...
  assert.equal(res.code,['asteroids','matching'].includes(game)?200:400);
 }
 assert.deepEqual(calls,['../lib/asteroids-scores','../lib/matching-scores']);
});
