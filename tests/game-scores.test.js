...
test('Shared leaderboard route dispatches each game and rejects unknown games',async()=>{
 ...
 for(const game of ['asteroids','matching','flappy','live','unknown',undefined]){
  ...
  assert.equal(res.code,['asteroids','matching','flappy','live'].includes(game)?200:400);
 }
 assert.deepEqual(calls,['../lib/asteroids-scores','../lib/matching-scores','../lib/flappy-scores','../lib/live-game']);
});
